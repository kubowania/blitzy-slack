...
test.describe('File Upload', () => {
  test.describe('Successful Upload', () => {
    test('uploads a PNG and renders it inline in the timeline within 5 seconds', async ({
      ...
      const channel = await createChannelViaApi(user.token, uniqueChannelName('upload'));
      ...
      // The FilePreview (AAP §0.5.4) renders the uploaded image inline; assert an
      // image preview becomes visible within the Gate 9 budget.
      await expect(
        page.locator('img').filter({ hasNot: page.locator('img[role="presentation"]') }).first(),
    ...
    test('shows a Sonner toast on upload', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('toast'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
  ...
  test.describe('Size Limit Enforcement', () => {
    test('rejects files larger than 10 MB', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('toobig'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
    ...
    test('client-side size check uses MAX_FILE_SIZE_BYTES from shared', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('boundary'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
  ...
  test.describe('Download', () => {
    test('file download link initiates a download', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('dl'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
  ...
  test.describe('Preview Rendering', () => {
    test('image attachments render inline as an image preview', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('inline'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
    ...
    test('non-image attachments render as a file card', async ({ page }) => {
      const user = await registerUserViaApi();
      const channel = await createChannelViaApi(user.token, uniqueChannelName('card'));
      await loginViaUi(page, user.email, user.password);
      await page.goto(`/app/channels/${channel.id}`);
