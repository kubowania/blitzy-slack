...
/**
 * Start (or resolve the existing) 1:1 DM with `participantUserId` by calling
 ...
 * @param participantUserId - the other participant's user id
 * @returns the created (or existing) DM id
 */
async function startDmViaApi(token: string, participantUserId: string): Promise<{ id: string }> {
...
test.describe('Direct Messages', () => {
  test.describe('Starting a DM', () => {
    test('starts a 1:1 DM with another user via the UI', async ({ page }) => {
      const { user1, user2 } = await registerTwoUsers();
  ...
  test.describe('Sending Messages in a DM', () => {
    test('sends and receives messages within a DM', async ({ page }) => {
      ...
      const dm = await startDmViaApi(user1.token, user2.userId);
  ...
  test.describe('Real-Time Delivery (DM)', () => {
    test('delivers DM messages bidirectionally between two clients', async ({ browser }) => {
      ...
      try {
        ...
        // user1 → user2: the message must arrive over Socket.io without a reload.
        ...
        // user2 → user1: delivery is symmetric within the same dm:<id> room.
  ...
  test.describe('Persistence and Reload', () => {
    test('DM persists across navigation away and back', async ({ page }) => {
      ...
      // Navigate away to the workspace home and confirm we left the DM route.
      await page.goto('/app');
  ...
  test.describe('Edge Cases', () => {
    test('rejects starting a DM with a non-existent user id', async () => {
      ...
      const response = await fetch(`${apiUrl}/api/dms`, {
        ...
        body: JSON.stringify({ participantUserId: fakeUserId }),
      });
    ...
    test('rejects starting a DM with malformed user id (Zod validation)', async () => {
      ...
      // `startDmSchema` validates `participantUserId` as a CUID, so a malformed
      ...
      const response = await fetch(`${apiUrl}/api/dms`, {
        ...
        body: JSON.stringify({ participantUserId: 'not-a-cuid' }),
      });
